...
function App() {
  ...
  const transferHandler = async (seller,_buyer,_id) => {
		try {
     
			await landContract.methods.transferFrom(seller,_buyer ,_id).send({ from: account })
			...
			window.alert('Error occurred when Transfering')
		}
	}
          ...
          {hasOwner && landOwner===account && (
            <div> 
              <div className='info--buyer'>
								<h2>Address</h2>
								<input type={"text"}  size="35"  onChange={getInputValue}/>
							</div>
						<button   onClick={() => transferHandler(account,buyer,landId)} className='button info--buy'>Transfer Property</button>
            </div>
					)}
				</div>
			)}
